class CrowdauthController < ApplicationController
  before_filter :configured?
  def index
    redirect_to :action => 'login'
  end

  def login
    if request.post?
      crowd_user = CrowdUser.new(params[:login][:username], params[:login][:password])
      unless crowd_user.exist?
        flash[:error] = 'login failed'
        return
      end

      user = User.first(:conditions => {:screen_name => crowd_user.name})
      user ||= User.new
      user.screen_name ||= crowd_user.name
      user.name ||= crowd_user.screen_name
      user.profile_image_url ||= 'data:image/gif;base64,'
      user.name = params[:login][:name] if not params[:login][:name].blank?
      user.profile_image_url = params[:login][:image_url] if not params[:login][:image_url].blank?
      user.save
      session[:current_user_id] = user.id
      redirect_to :controller => 'chat', :action => 'index'
    end

  end

  private
  def configured?
    unless Setting[:login_link]
      logger.info "not configured!!"
      render :file => 'public/404.html'
    end
  end
end
